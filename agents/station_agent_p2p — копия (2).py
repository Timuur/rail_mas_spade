...
class StationAgent(Agent):
    ...
    class ListenHello(CyclicBehaviour):
        async def run(self):
            ...
            # Mark this station as alive
            if st_id not in self.agent.ready_stations:
                print(f"[{self.agent.station_id}] sees station alive: {st_id}")
            ...
            # If we've seen all stations → set barrier
    ...
    class ListenCFP(CyclicBehaviour):
        async def run(self):
            ...
            offer = await self.agent._try_build_offer(od, proposer_jid=str(self.agent.jid))
            if not offer:
    ...
    # --------- Control queue (proposals, informs) ---------
    class ListenControl(CyclicBehaviour):
        ...
        async def run(self):
            ...
            if mtype == TYPE_ORDER_PROPOSE:
                ...
                payload = json.loads(msg.body)
                print(f"[{self.agent.station_id}] INFORM по {payload.get('order_id')} — записываю результат.")
                plan = payload["result"]
                await self.agent._append_schedule(plan)
        ...
        async def run(self):
            ...
            for od in my_orders:
                ...
                if first:
                    ...
                    print(
                        f"[{self.agent.station_id}] Delay {delay:.2f}s before first CFP (order {od['id']})"
                    )
                ...
                # notify participants (accept/reject)
                for of in offers:
                    m = Message(to=of["proposer_jid"])
                    if of is best:
                        m.set_metadata("type", TYPE_ORDER_ACCEPT)
                        m.body = json.dumps({"order_id": od["id"], "chosen_offer": best},ensure_ascii=False)
                    else:
                ...
                for s in recipients:
                    ...
                    im.body = json.dumps({"order_id": od["id"], "result": best},ensure_ascii=False)
                    await self.send(im)
    ...
    class IdleKiller(CyclicBehaviour):
        ...
        async def run(self):
            if time.time() - self.agent.last_active > self.timeout_s:
                print(f"[{self.agent.station_id}] Agent idle {self.timeout_s}s — stopping.")
    ...
    async def _try_build_offer( self, od: Dict[str, Any], proposer_jid: str) -> Optional[Dict[str, Any]]:
        origin = od["from"]
        dest = od["to"]
        cargo_type = od["type"]
        ...
        if remaining > 1e-6:
            extra, latest_extra_eta, extra_cost = await self._gather_wagons(od, remaining)
            if not extra:
        ...
        for l in sorted(locos, key=lambda x: float(x["cost_per_km"])):
            if float(l["max_tonnage"]) >= total_capacity:
                ...
                break
        if not loco:
            print(f"[{self.station_id}] Нет подходящей тяги ({total_capacity} т) для {od['id']}")
            return None
        ...
        print(f"[{self.station_id}] Состав {od['id']}: локо={loco['id']}, вагонов={len(offer['wagons'])}, "
              f"cap={total_capacity} т, cost={offer['total_cost']}, T={offer['depart_h']}→{offer['arrive_h']}")
        return offer
