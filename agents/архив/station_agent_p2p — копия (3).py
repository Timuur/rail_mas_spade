...
class StationAgent(Agent):
    def __init__(
        ...
        **kwargs,
        ...
        self.trading_started: bool = False  # <- новый флаг
    ...
    class ListenControl(CyclicBehaviour):
        async def run(self):
            ...
            if mtype == TYPE_ORDER_PROPOSE:
                ...
                payload = json.loads(msg.body)
                print(
                    f"[{self.agent.station_id}] INFORM по {payload.get('order_id')} "
        ...
        async def run(self):
            ...
            for od in my_orders:
                ...
                for s in recipients:
                    ...
                    im.body = json.dumps(
                        {"order_id": od["id"], "result": best}, ensure_ascii=False
                    )
                    await self.send(im)
    ...
    async def _gather_wagons(
        ...
        # Broadcast wagon_cfp (используем behaviour для send)
        ...
        while asyncio.get_event_loop().time() < deadline:
            msg = await ctx.receive(timeout=0.5)
            if not msg:
                continue
            meta = msg.metadata or {}
            mtype = meta.get("type")
            if mtype != TYPE_WAGON_PROPOSE:
                # складываем в общую очередь агента (bus хранится только на агенте)
